...
from learners.learners_moe import *
from client import *
...
def get_data_dir(args):
    ...
    """
    data_dir = os.path.join("data", args.experiment, f"segmented_data/n_task-{args.n_tasks}_alpha-{args.alpha}")
...
def get_learner(
    ...
    if name == "synthetic":
        ...
        if method == "pFedMM":
            if is_classifier:
                ...
                model = MobileNetFeature().to(device)
        elif method == "pFedMoE":
            if is_classifier:
                model = get_mobilenet(n_classes=10).to(device)
            else:
                model = GateNetwork(m=n_learners, input=3*32*32).to(device)
        else:
            model = get_mobilenet(n_classes=10).to(device)
        is_binary_classification = False
    elif name == "cifar100":
        ...
        if method == "pFedMM":
            if is_classifier:
                model = MobileNetClassifier(num_classes=100).to(device)
            else:
                model = MobileNetFeature().to(device)
        elif method == "pFedMoE":
            if is_classifier:
        ...
        if method == "pFedMM":
            if is_classifier:
                model = EmnistClassifier(num_classes=62).to(device)
            else:
                model = EmnistFeature().to(device)
        elif method == "pFedMoE":
            if is_classifier:
                model = FemnistCNN(num_classes=62).to(device)
            else:
                model = GateNetwork(m=n_learners, input=28*28).to(device)
        else:
            model = FemnistCNN(num_classes=62).to(device)
        is_binary_classification = False
    ...
    if name == "shakespeare":
        ...
        if method == "pFedMM" or method == "pFedMoE":
            return Expert(
                model=model,
                criterion=criterion,
                metric=metric,
...
def get_learners_ensemble(
    ...
    if method == "pFedMM":
        learners.append(get_learner(
            method=method,
            name=name,
            device=device,
            optimizer_name=optimizer_name,
            scheduler_name=scheduler_name,
            initial_lr=initial_lr*n_learners,
            input_dim=input_dim,
            output_dim=output_dim,
            n_rounds=n_rounds,
            seed=seed + n_learners,
            mu=mu,
            is_classifier=False
            )
        )
    ...
    if name == "shakespeare":
        ...
        if method == "pFedMM":
            return LearnersMoE(moe=learners, learners_weights=learners_weights.to(device))
        elif method == "pFedMoE":
            return LearnerspFedMoE(moe=learners, learners_weights=None)
        else:
...
def get_aggregator(
    ...
    elif aggregator_type == "centralizedpFedMoE":
        return CentralizedAggregatorpFedMoE(
            clients=clients,
            global_learners_ensemble=global_learners_ensemble,
            log_freq=log_freq,
            global_train_logger=global_train_logger,
