...
def run_experiment(args_):
    ...
    data_dir = get_data_dir(args_)

    if "logs_dir" in args_:
        logs_dir = args_.logs_dir
    else:
        logs_dir = os.path.join("logs", args_to_string(args_), f"n_task-{args_.n_tasks}_n_learners-{args_.n_learners}__alpha-{args_.alpha}")
...
# python run_experiment.py emnist FedEM --n_learners 3 --n_tasks 100 --n_rounds 100 --bz 128 --lr 0.03  --lr_scheduler multi_step --log_freq 1 --device cuda --optimizer sgd --seed 1234 --verbose 1