            ...
            for client_id, client in enumerate(clients):
                ...
                if self.verbose > 1:
                    ...
                    with np.printoptions(precision=3, suppress=True):
                        print("Pi: ", client.learners_ensemble.learners_weights.numpy())
...
class CentralizedAggregatorpFedMoE(CentralizedAggregator):
    def update_clients(self):
        for client in self.clients:
            for learner_id, learner in enumerate(client.learners_ensemble[:-2]):
                copy_model(learner.model, self.global_learners_ensemble[learner_id].model)
